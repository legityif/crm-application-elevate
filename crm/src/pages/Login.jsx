...
function Login() {
  ...
  useEffect(() => {
    ...
    google.accounts.id.initialize({
      client_id: "718066585548-9c25n26k4ci4do8f3mh45stub0s4de0u.apps.googleusercontent.com",
  ...
  const handleCredentialResponse = async (response) => {
    ...
    console.log(payload);
    // payload has sub (google id), email, name, picture, etc.
    if (await userExists(payload.sub, "123123123123")) {
      navigate(`/dashboard?username=${payload.sub}`);
    } else {
      postUser(payload.sub, "123123123123", payload.email);
      navigate(`/dashboard?username=${payload.sub}`);
    }
  };
  ...
  return (
    <LoginContainer>
      ...
      <LoginForm>
        ...
        <OtherLoginOptions>
          <IconOnlyButton onClick={() => window.google && window.google.accounts.id.prompt()}>
